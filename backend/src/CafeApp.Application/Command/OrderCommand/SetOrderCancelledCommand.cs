...
{
    public sealed record SetOrderCancelledCommand(Guid Id) : IRequest<Result<ActiveOrderDetailsDto>>;

    internal sealed class SetOrderCancelledCommandHandler(IOrderRepository orderRepository, IHttpContextAccessor httpContextAccessor, IUnitOfWork unitOfWork) : IRequestHandler<SetOrderCancelledCommand, Result<ActiveOrderDetailsDto>>
    {
        public async Task<Result<ActiveOrderDetailsDto>> Handle(SetOrderCancelledCommand request, CancellationToken cancellationToken)
        {
            ...
            if (order.Status == OrderStatus.Served || order.Status == OrderStatus.Paid)
                return Result<ActiveOrderDetailsDto>.Failure("Bu sipariş iptal edilemez!");
